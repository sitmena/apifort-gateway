...
//                if(superAdminApiKey.equals(apiKey)){
                    Jwts.parserBuilder()
                            .setSigningKey(AppLifecycleBean.getPublicKey())
                            .build()
                            .parseClaimsJws(token.replaceAll(ApiFort.API_FORT_JWT_TOKEN_PREFIX, ApiFort.API_FORT_EMPTY_STRING));
//                }else{
//                    String certificate =redisClient.get(apiKey).toString();
//                    log.info("Cache Certificate is {}", certificate);
